...
class ChatResponse:
    ...
    --------
    >>> # Without prefill
    >>> response = ChatResponse(prefill="", generated="Hello! How can I help?")
    >>> str(response)
    'Hello! How can I help?'
    >>> response.has_prefill
    False

    >>> # With JSON prefill
    >>> response = ChatResponse(prefill='{"answer": "', generated='42"}')
    >>> response.full_text()
    '{"answer": "42"}'
    >>> response.to_message()
    ...
    def full_text(self) -> str:
        ...
        -------
        str
            Concatenation of prefill and generated text.
        """
    ...
    def to_message(self, role: str = "assistant") -> dict[str, str]:
        ...
        This is useful for multi-turn conversations where you want to append
        the model's response to the message list.

        Parameters
        ----------
        role : str, default="assistant"
            Message role for the returned dict.
        ...
        Examples
        --------
        >>> response = ChatResponse(prefill="", generated="Paris")
        >>> messages.append(response.to_message())
        >>> # messages now contains: [..., {"role": "assistant", "content": "Paris"}]
        """
        ...
        **sampling_kwargs : Any
        ...
        async with self._steering_rwlock.read_lock():
            ...
            for i, messages_conv in enumerate(batched_messages):
                # Detect if using assistant prefill
                has_prefill = False
                prefill_text = ""
                if (
                    messages_conv
